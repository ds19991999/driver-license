...
class Surface(ttk.Frame):
    ...
    def show_roi(self, r, roi, color):  # 显示车牌图像
        if r:   # 如果识别出车牌
            ...
            try:
                ...
                self.color_ctl.configure(text=c[0], background=c[1], state='enable')    # 显示车牌号背景颜色
            except:
    ...
    def from_pic(self):     # 从图片识别车牌
        ...
        if self.pic_path:
            ...
            r, roi, color = self.predictor.predict(img_bgr)     # 识别图像中的车牌信息
            self.show_roi(r, roi, color)    # 在窗口显示识别的车牌信息
