...
# 来自opencv的sample，用于svm训练   进行仿射变换
def deskew(img):
    m = cv2.moments(img)
    if abs(m['mu02']) < 1e-2:
        return img.copy()
...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        if os.path.exists("Core//SVM/svm.dat"):
            self.model.load("Core/SVM/svm.dat")
        else:
            chars_train = []
            chars_label = []
            ...
            for root, dirs, files in os.walk("train\\chars2"):
                ...
                for filename in files:
                    ...
                    digit_img = cv2.imread(filepath)   # 读入图片
                    digit_img = cv2.cvtColor(digit_img, cv2.COLOR_BGR2GRAY) #灰度化
                    chars_train.append(digit_img)  # 加入训练集
                    # chars_label.append(1)
                    chars_label.append(root_int)

            chars_train = list(map(deskew, chars_train))   # 对所有图进行仿射变换
        ...
        # 中文字符
        if os.path.exists("Core/SVM/svmchinese.dat"):
            self.modelchinese.load("Core/SVM/svmchinese.dat")
        else:
            chars_train = []
            chars_label = []
            ...
            # chars_train = chars_train.reshape(-1, 20, 20).astype(np.float32)
            chars_label = np.array(chars_label)
            print("C_chars_train.shape",chars_train.shape)
            self.modelchinese.train(chars_train, chars_label)

    def save_traindata(self):
        if not os.path.exists("Core/SVM/svm.dat"):
            self.model.save("Core/SVM/svm.dat")
        if not os.path.exists("Core/SVM/svmchinese.dat"):
            self.modelchinese.save("Core/SVM/svmchinese.dat")
    ...
    def accurate_place(self, card_img_hsv, limit1, limit2, color):
        ...
        col_num_limit = col_num * 0.8 if color != "green" else col_num * 0.5  # 绿色车牌颜色有渐变
        # 找（字符？）上下界
    ...
    def predict(self, car_pic):
        ...
        # showimg = cv2.resize(img, (int(pic_width * 0.5), int(pic_hight * 0.5)), interpolation=cv2.INTER_AREA)
        ...
        # cv2.imshow('after opening-close and open', img_edge2)     # 闭操作和开操作后图像
        ...
        for card_index, card_img in enumerate(card_imgs):
            ...
            # cv2.imshow('card_imgs', card_imgs[card_index])   # 识别出的图片图像
...
if __name__ == '__main__':
    c = CardPredictor()
    c.train_svm()
    r, roi, color = c.predict("Test/car.jpg")
    province = r[0]
    card = ""
    for i in r[1:]:
        card += i
